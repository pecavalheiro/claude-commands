...
1. Setup — **prior-run check FIRST:** read `requirements/.current-requirement` and scan every `requirements/*/metadata.json` + `00-initial-request.md` for an existing run of the same ticket/topic — **match by ticket ID, not slug** (the same ticket can produce different slugs). If one exists:
   - ACTIVE run for the same ticket → resume it (continue from its recorded phase); do not create a new folder.
   - Prior run with unresolved `holds` in metadata (a critical answer was outstanding) → ask me whether the owner has answered; record the answer in `05-detail-answers.md` and finish the run from where it paused.
   - Prior run WITH recorded answers (02/05 files) → **only answers I actually gave** inherit as decisions-made-once, with provenance "inherited from <folder>", and are never re-asked. Model-recommended defaults, comms defaults, and interim decisions inherit as DRAFTS and re-enter the criticality override.
   - Prior COMPLETE run → ask me: extend it in place, or supersede it with a fresh run.
   - Prior abandoned run (questions drafted, no answers) → inherit its findings/questions as draft inputs to your gates (not obligations), and supersede it.
   - Any folder being replaced gets `"status": "superseded"` written into its metadata so /synthesize never picks it; the new folder's metadata records `"supersedes": "<folder>"`.
   Only then create `requirements/YYYY-MM-DD-HHMM-[slug]/` (slug from $ARGUMENTS), write `00-initial-request.md` (the request verbatim + a summary of the primary source + any inheritance notes), `metadata.json` (structure below — record the primary ticket id in `sources` immediately, so future prior-run scans can match), and update `requirements/.current-requirement`.
...
19. Write `06-requirements-spec.md` under the **output contract**:
    ...
    - **Holds:** while any `metadata.holds` entry has `resolution: null` (override option (a), pause variant), `06-requirements-spec.md` is NOT written and `phase` is NEVER set to "complete" — the run waits (resume path: re-running /requirements-start on the ticket finds the blocked run via the prior-run check and asks whether the owner answered), or I descope the held FR and the spec completes without it.
...
```json
{
  ...
  "status": "active | complete | incomplete | superseded",
  "supersedes": null,
