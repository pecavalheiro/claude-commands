...
### The all-lenses barrier — the review is written once, after ALL lenses report

The harness may run the lenses as background tasks: each launch returns immediately, and a lens's findings arrive later as a task notification. Notifications are delivered only **between** your turns — one can never interrupt a turn in progress. So mid-turn silence is not evidence a lens is still running: it may have finished minutes ago with its result queued, invisible until your turn ends. Never reason "lens X has not returned" from inside a turn; you cannot know that.

- The review — ratings, ranked findings, the entire Output section — is produced **exactly once**, and only after every lens has reported. No partial or provisional review, no "covering" a missing lens yourself, no publish-then-correct. A review that contradicts itself minutes later is far worse than a review that arrives late: the retraction, not the delay, is the failure mode this barrier exists to prevent.
- While lenses are pending, use the time on Step 3 verification of findings already received. When you run out of that work, **end the turn with a one-line status** ("4 of 5 lenses in; waiting on correctness") and nothing else. Ending the turn is precisely what lets the next queued result reach you — the harness re-invokes you automatically when a lens reports. Publishing early buys zero wall-clock time: the straggler's queued result lands the second your turn ends either way.
- Never conclude a lens is dead or "too slow" from elapsed time; lens runtimes legitimately differ by 2–3×, and a lens that looks stalled has usually just not been deliverable mid-turn. If you genuinely suspect a failure, check the task's status through the harness (its task tools or output file — not TaskList, which tracks to-do items, not agents). If a lens has truly failed, STOP and tell me — relaunching it or proceeding on four lenses is my call, never a silent default.

## Step 3: verify, dedup, rank — yours alone, never delegated

Verification of individual findings may start as each lens reports, but ranking and the Output section wait for the all-lenses barrier: not one word of the review is written while any lens is outstanding.
