...
Runtime files a command reads (not commands themselves) live in a top-level directory, which the installer symlinks to `~/.claude/<dirname>`: `requirements-phases/` (the pipeline's phase rules), `review-lenses/` (`/final-check`'s lenses). Commands reference them by their `~/.claude/<dirname>` path. `commands/` and `docs/` are exempt from this rule.
...
- `commands/final-check.md` and `review-lenses/` are the live source; the copies in the dotfiles repo are a frozen backup — don't edit those.
